...
from ai.itinerary import router as itinerary_router   # ✅ safe now
...
# -----------------------------
app.include_router(itinerary_router)   # ✅ include itinerary routes
...
def get_destinations():
    return app.state.destinations.to_dict(orient="records")
...
def get_recommendations(req: RecommendationRequest):
    ...
    filtered = destinations[
        ...
        (destinations["season"].str.lower() == season)
    ]

    if filtered.empty:
        return {"message": f"No {req.category} places found in {req.location}"}

    # ✅ Drop image_url column before returning
    filtered = filtered.drop(columns=["image_url"], errors="ignore")

    return filtered.head(req.duration).to_dict(orient="records")
